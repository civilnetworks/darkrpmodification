...
DarkRP.createEntity("Drone", {
	...
	cmd = "/BuyDrone",
	customCheck = function(ply)
        return ply:Team() == TEAM_DIRECTOR_OF_EXTERNAL_AFFAIRS or ply:Team() == TEAM_DEPUTY_DIRECTOR_OF_EXTERNAL_AFFAIRS or ply:Team() == TEAM_EXTERNAL_AFFAIRS_AGENCY_MANAGER or ply:Team() == TEAM_EXTERNAL_AFFAIRS_SPECIAL_AGENT or ply:Team() == TEAM_EXTERNAL_AFFAIRS_SENIOR_AGENT
	end,
	CustomCheckFailMsg = function(ply, entTable) 
...
/*DarkRP.createEntity("Diamond Printer", {
	...
	customCheck = function(ply)
		return CanBuyPrinters(ply) and ply:AtLeastRanked("Keter VIP")
	end,
	CustomCheckFailMsg = function(ply, entTable)
		if(!CanBuyPrinters(ply))then
			return "You must be civilian to purchase this!"
		end

		return "You must be at least Keter VIP to purchase this printer!"
	end,
})*

// Rare items //

DarkRP.createEntity("SCP-3078 Instance", {
	ent = "scp_3078",
	model = "models/props_lab/frame002a.mdl",
	price = 25,
	max = 2,
	category = "Anomalous Objects",
	cmd = "/BuySCP3078",
	allowed = {TEAM_MCD},
})

DarkRP.createEntity("SCP-099 Reproduction", {
	ent = "scp_3078",
